/**
 * lamaPixelStudio - JPG to PNG Converter Engine
 * Converts standard JPEG images into lossless PNG graphics locally in the browser.
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function handleFiles(incoming) {
    Array.from(incoming).forEach(file => {
      if (!file.type.includes('jpeg') && !file.type.includes('jpg') && !file.name.toLowerCase().match(/\.(jpg|jpeg)$/)) return;
  ...
  function renderUI() {
    if (state.files.length > 0) {
      ...
      convertAllBtn.textContent = `Convert All to PNG (${state.files.length})`;
    } else {
      emptyNote.style.display = 'block';
      convertAllBtn.disabled = true;
      convertAllBtn.textContent = 'Convert All to PNG';
    ...
    state.files.forEach(item => {
      ...
      card.innerHTML = `
        ...
        <div style="display:flex; gap:10px; align-items:center;">
          ...
          <div style="flex:1; font-size:12px; color:var(--ink-soft); line-height:1.5;">
            <div>Original JPG: <strong>${formatBytes(item.originalSize)}</strong></div>
            <div>PNG Size: <strong style="color:var(--emerald);">${item.convertedSize ? formatBytes(item.convertedSize) : 'Pending'}</strong></div>
          </div>
        </div>
        <div style="display:flex; gap:8px;">
          <button type="button" class="btn btn-accent btn-small btn-block" data-action="download" ${!item.convertedUrl ? 'disabled' : ''}>Download PNG</button>
        </div>
      `;
      ...
      card.querySelector('[data-action="download"]').addEventListener('click', () => {
        const cleanName = item.name.replace(/\.(jpg|jpeg)$/i, '') + '.png';
  ...
  if (downloadZipBtn) {
    downloadZipBtn.addEventListener('click', async () => {
      ...
      const zip = new JSZip();
      const folder = zip.folder('lamapixelstudio-pngs');
      ...
      a.download = 'lamapixelstudio-pngs.zip';
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
    });
