/**
 * lamaPixelStudio - WebP to JPG Converter Engine
 * Handles flattening WebP transparent files and batch converting them into standard JPEGs.
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Background color swatch selector for transparency flattening
  ...
  function handleFiles(incoming) {
    Array.from(incoming).forEach(file => {
      if (!file.type.includes('webp') && !file.name.toLowerCase().endsWith('.webp')) return;
  ...
  function renderUI() {
    ...
    state.files.forEach(item => {
      ...
      card.innerHTML = `
        ...
        <div style="display:flex; gap:10px; align-items:center;">
          ...
          <div style="flex:1; font-size:12px; color:var(--ink-soft); line-height:1.5;">
            <div>Original WebP: <strong>${formatBytes(item.originalSize)}</strong></div>
      ...
      card.querySelector('[data-action="download"]').addEventListener('click', () => {
        const cleanName = item.name.replace(/\.webp$/i, '') + '.jpg';
      ...
      for (let item of state.files) {
        await new Promise((resolve) => {
          ...
          img.onload = () => {
            ...
            // Fill background color for webp transparency flattening
